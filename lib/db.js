...
export async function queryListings(params = {}) {
  const {
    ...
    minPrice, maxPrice, maxMileage, minYear, maxYear,
  ...
  // year is stored as text; restrict to well-formed 4-digit years then compare as strings (works for YYYY).
  if (minYear) add("(year ~ '^[0-9]{4}$' AND year >= ?)", String(minYear));
  if (maxYear) add("(year ~ '^[0-9]{4}$' AND year <= ?)", String(maxYear));
