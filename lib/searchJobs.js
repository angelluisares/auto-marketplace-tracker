...
import metroStore from './metroStore.js';
...
export async function createAndRun(text, region) {
  const parsed = parseSearch(text);
  const reg = grid.isRegion(region) ? region : 'all';
  const cities = await metroStore.enabledCitiesFor(reg);
  const id = String(seq++);
  const job = {
    id, text, parsed, region: reg, cities,
...
async function finish(job) {
  ...
  try { await store.recordRun(job.parsed, job.found, job.region, job.durationMs); } catch { /* history is best-effort */ }
}
