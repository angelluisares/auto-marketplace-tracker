...
const pg = require('./pg');
const { parseSearch } = require('./searchParse');
const { query } = pg;
...
function rowToParsed(row) {
  // Re-parse the original text so saved/scheduled searches automatically benefit
  // from parser improvements. (Stored parsed columns are kept only for display.)
  return parseSearch(row.text);
}
...
async function matchListings(parsed, limit = 200) {
  ...
  const STOP = new Set(['the', 'a', 'an', 'for', 'sale', 'in', 'of', 'to', 'my', 'want', 'looking', 'need', 'used', 'new', 'or', 'and', 'with', 'near']);
  const where = [];
  const params = [];
  (q || '').split(/\s+/).filter(Boolean).filter(t => !STOP.has(t.toLowerCase())).forEach(t => { params.push('%' + t + '%'); where.push(`raw_text ILIKE $${params.length}`); });
