...
async function recordRun(parsed, found) {
  await ensureSearchTable();
  await query(
    `INSERT INTO searches (text, query, max_miles, max_price, min_year, max_year, last_run_at, run_count, last_found)
    ...
    [parsed.raw, parsed.query, parsed.maxMiles, parsed.maxPrice, parsed.minYear, parsed.maxYear, found]
  );
}
